...
typedef struct gtp_msg_type_map {
	const char *name;
...
/* GTPv1 */
int gtp1_ie_apn_extract(gtp1_ie_apn_t *, char *, size_t);
size_t gtp1_get_header_len(gtp1_hdr_t *);
uint8_t *gtp1_get_ie_offset(uint8_t, uint8_t *, uint8_t *);
uint8_t *gtp1_get_ie(uint8_t type, pkt_buffer_t *);
size_t gtp1_ie_add_tail(pkt_buffer_t *, uint16_t);
...
size_t gtpc_get_header_len(gtp_hdr_t *);
int gtp_imsi_rewrite(gtp_apn_t *, uint8_t *);
int gtp_ie_imsi_rewrite(gtp_apn_t *, uint8_t *);
int gtp_ie_apn_labels_cnt(const char *, size_t);
int gtp_apn_extract_ni(char *, size_t, char *, size_t);
int gtp_ie_apn_extract_ni(gtp_ie_apn_t *, char *, size_t);
int gtp_ie_apn_extract_oi(gtp_ie_apn_t *, char *, size_t);
int gtp_ie_apn_extract_plmn(gtp_ie_apn_t *, char *, size_t);
int gtp_ie_apn_rewrite_oi(gtp_ie_apn_t *, size_t, char *);
int gtp_ie_apn_rewrite(gtp_apn_t *, gtp_ie_apn_t *, size_t);
int gtp_ie_f_teid_dump(gtp_ie_f_teid_t *);
int gtp_dump_ie(uint8_t *, size_t);
uint8_t *gtp_get_ie_offset(uint8_t, uint8_t *, size_t, size_t);
uint8_t *gtp_get_ie(uint8_t, pkt_buffer_t *);
int gtp_foreach_ie(uint8_t, uint8_t *, size_t, uint8_t *,
		   gtp_server_t *, gtp_session_t *, int, void *,
		   gtp_teid_t * (*hdl) (gtp_server_t *, gtp_session_t *, int, void *, uint8_t *));
ssize_t gtpu_get_header_len(pkt_buffer_t *);
