...
/*
 *	Convert IPv6 address (struct in6_addr) to string representation
 *	Returns formatted IPv6 string with proper compression of zero sequences
 */
char *
addr_stringify_in6_addr(struct in6_addr *addr, char *buffer, size_t buflen)
{
	const uint16_t *words;
	int zero_start = -1, zero_len = 0;
	int best_start = -1, best_len = 0;
	int i, len = 0;
	bool compressed = false;

	if (!addr || !buffer || buflen < INET6_ADDRSTRLEN)
		return NULL;

	words = (const uint16_t *)&addr->s6_addr;
	...
	for (i = 0; i < 8; i++) {
		if (ntohs(words[i]) == 0) {
			...
			if (zero_len > best_len && zero_len > 1) {
	...
	for (i = 0; i < 8; i++) {
		...
		/* Add the hex word */
		len += snprintf(buffer + len, buflen - len, "%x", ntohs(words[i]));
